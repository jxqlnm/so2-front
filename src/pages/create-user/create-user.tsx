...
export function CreateUser() {
  ...
  return (
    <>
      {modal && <ConfirmationModal mensagem="Usuário criado com sucesso!" />}
      {modalError && <ModalError mensagem="Usuário já existe!" />}
      ...
      <div className="flex items-start justify-center mt-4 font-medium space-x-5">
        <div>
          <form className="space-y-4 justify-items-start">
            ...
            <SelectionBox
              nome="Nível"
              lista={Object.values(LEVEL)}
              valor={level}
              setValor={setLevel}
            />
            <button className="btn" onClick={(e) => criarUsuario(e)}>
              Salvar
            </button>
          </form>
        </div>
